...
void TC_Config::parse(istream &is)
{
    ...
    while(getline(is, line))
	{
		...
		else if(line[0] == '<')
		{
			...
			if(posl == string::npos)
			{
				throw TC_Config_Exception("[TC_Config::parseFile]:parse error! line : " + line);
			}
			...
			if(line[1] == '/')
			{
                ...
                {
					throw TC_Config_Exception("[TC_Config::parseFile]:parse error! <" + sName + "> hasn't matched domain.");
                }

                if(stkTcCnfDomain.top()->getName() != sName)
				{
					throw TC_Config_Exception("[TC_Config::parseFile]:parse error! <" + stkTcCnfDomain.top()->getName() + "> hasn't match <" + sName +">.");
				}
	...
	{
		throw TC_Config_Exception("[TC_Config::parseFile]:parse error : hasn't match");
    }
}
