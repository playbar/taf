<?php
/*
 * author guanpingli @tencent 
 * 2010/09/20
 *jce库 .定义了集中wup支持的类的具体实现 
 * */
 
require_once ('jce_exception.php');
require_once ('jce_header.php');
define("JCE_MAXIMUM_TAG",255);
...
{                                          
    ...
    public function _unpack_base(&$stream,$type,&$val)
    {
        ...
        }else if($type == 'c_char')
        {
            ...
            throw new JCEException(__LINE__, HEADER_TYPE_ERROR);
        }        
    } 

    private function _unpack_length(&$stream)
	{
		jce_header::_unpack_header($stream,$type,$this_tag);
		//length tag must be 0
		if(0 != $this_tag)
        {
            if($isRequire == true){
                throw new JCEException(__LINE__, TAG_NOT_MATCH);   
            }else{
                return;
            }
        }     
 	...
 	public function _skip_struct(&$stream)
    {
        ...
        do{
        	...
        	jce_header::_peek_header($stream,$struct_type,$this_tag);
        	$this->_skip_base($stream);        	
        ...
        if($struct_type != 'c_struct_end')
            throw new JCEException(__LINE__, TYPE_NOT_MATCH);
    }
...
class c_char extends data_type
{
    ...
    public function __set($name, $val)
    {
        ...
        if($name == 'val')
        {
            ...
            {
                if($val > 127 || $val < -128)
                    throw new JCEException(__LINE__, OUTOF_RANGE);                
                $this->val = chr($val);    
            }else if(is_null($val)){
                $this->val = chr(0);
                ...
                if(strlen($val) != 1)
                    throw new JCEException(__LINE__, LEN_NOT_MATCH);
                $this->val = $val;   
            }                                           
        }else{
            throw new JCEException(__LINE__, VAR_NOT_FOUND);
        }    
    }    
    ...
    public function write(&$stream,$tag)
    {
        ...
        {
        	//先把类型信息打包
            jce_header::_pack_header($stream,'c_char',$tag);
           	//直接把值放到stream后面
            $stream.=$this->val;
        }else{
            jce_header::_pack_header($stream,'c_zero',$tag);
        }        
    }
    //从stream里读一个char出来
    ...
    {                
        ...
        jce_header::_peek_header($stream,$type,$this_tag,$isRequire);
        if($tag != $this_tag)
        {
            if($isRequire == true)
            {
...
class c_short extends data_type
{
    ...
    public function write(&$stream ,$tag)
    {
        if($this->val >= (-128) && $this->val <= 127){            
            ...
            jce_header::_pack_header($stream,'c_short',$tag);
...
class c_int extends data_type
{
    ...
    public function write(&$stream,$tag)
    {
        ...
        if($this->val >= (-32768) && $this->val <= 32767){
        	...
        	//这里进行int类型的打包
            jce_header::_pack_header($stream,'c_int',$tag);
            $stream .= pack('N',$this->val);
        }
    }
    public function read(&$stream,$tag,$isRequire = true)
...
class c_int64 extends data_type
{
    ...
    public function write(&$stream,$tag)
    {
        if((bccomp($this->val,"-2147483648") >= 0) && (bccomp($this->val,"2147483647") <= 0))
        {
            ...
            jce_header::_pack_header($stream,'c_int64',$tag);
...
class c_string extends data_type
{
    ...
    public function write(&$stream,$tag)
    {
        ...
        {
        	//用4字节表示字符串长度
            jce_header::_pack_header($stream,'c_string4',$tag);            
            $stream.= pack("N",$len);
            $stream.= pack("A*",$this->val);
            
        }else{
        	//用一个字节表示字符串长度
            jce_header::_pack_header($stream,'c_string1',$tag);
...
class c_vector extends data_type
{
    ...
    public  function __construct($proto)
    {
        ...
        if(is_null($proto))
        {
            throw new JCEException(__LINE__, PROTO_TYPE_UNKNOWN);
        }
    ...
    public function write(&$stream,$tag)
    {
        ...
        if($this->class_name == 'c_char')
        {
            ...
            jce_header::_pack_header($stream,'c_simple_list',$tag);
            jce_header::_pack_header($stream,'c_char',0);
            $len = new c_int;
        	...
        	//type=c_list
            jce_header::_pack_header($stream,'c_list',$tag);
            $len = new c_int;
            $len->val = $cnt;
            $len->write($stream,0);            
            for($i = 0;$i < $cnt;$i++)
...
class c_map extends data_type
{
    ...
    {
        $cnt = count($this->map);        
        jce_header::_pack_header($stream,'c_map',$tag);
...
abstract class c_struct extends data_type
{
    ...
    public function __set($name,$val)
    {
        ...
        throw new JCEException(__LINE__, OBJECT_NOT_FOUND);               
    }
    ...
    public function write(&$stream,$tag)
    {
        ...
        jce_header::_pack_header($stream,'c_struct_begin',$tag);
        $this->_pack_every_elem($stream);       
        //tag必须是0
        jce_header::_pack_header($stream,'c_struct_end',0);
    ...
    {    	
    	...
    	while(1)
    	{
    		if($tagMax >= JCE_MAXIMUM_TAG)
    		{
    			throw new JCEException(__LINE__, TAG_NOT_MATCH);   
    		}
    		jce_header::_peek_header($stream,$type,$this_tag);
