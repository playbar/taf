<?php
/*
 * author guapingli @tencent
 *  2010/09/20
 *  jce库，定义类型信息。以及基本的header里的tag和type信息的打包和解包实现
 * */
class jce_header
{
    static private $type;
    ...
    static function _pack_header(&$stream,$type,$tag)
    {
        ...
        //tag必须是数字
        if(!is_int($tag))
            throw new JCEException(__LINE__, HEADER_TAG_ERROR);
        
       	//检查type是否合法。除了c_char 之外的不合法类型名字都会type中取的0值
        if($type != 'c_char' && self::$type[$type] == 0)
            throw new JCEException(__LINE__, HEADER_TYPE_UNKNOWN);       
